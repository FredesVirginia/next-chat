  ...
  return (
    <div className='flex-2'>
      <div className='hidden lg:block overflow-auto'>
        ...
        <div className='block bg-gray-300 mt-5 sm:hidden overflow-auto'>
        {Object.entries(chats)?.sort((a, b) => b[1].date - a[1].date).map((chat) => (
          <div
            className='px-4 mt-2 my-10 lg:my-0 flex items-center gap-3 text-white cursor-pointer bg-gray-300 hover:bg-color1 transition'
            key={chat[0]}
            onClick={() => handleSelect(chat[1].userInfo)}
          >
            <img src={chat[1].userInfo.photoURL} alt="" className='w-[50px] rounded-full' />
            <div >
              <span className=' font-bold  text-xl text-gray-700'> {chat[1].userInfo.displayName} </span>
              <p className='overflow-hidden font-bold text-gray-600  overflow-ellipsis max-w-[150px]'> {chat[1].lastMessage?.text} </p>
            </div>
          </div>
        ))
        }</div>
